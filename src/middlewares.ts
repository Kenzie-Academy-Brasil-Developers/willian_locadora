...
const idExists = async (req: Request, res: Response, next: NextFunction): Promise<Response|void> => {
    const queryResult: QueryResult = await client.query('SELECT * FROM "movis" WHERE "id" = $1;',
    [req.params.id]);
    ...
    res.locals = { ...res.locals, foundMovis: queryResult.rows[0]}
    return next();
};

const nameExists = async (req: Request, res: Response, next: NextFunction): Promise<Response|void> => {
    const queryResult: QueryResult = await client.query('SELECT * FROM "movis" WHERE "name" = $1;',
    [req.body.name]);
